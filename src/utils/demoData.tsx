...
export const PollComponent: React.FC<StoryItemControls> = ({ pause, resume, next }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        {['Swipe Gestures', 'Auto-advance', 'Custom Components', 'Video Support'].map((option, idx) => (
          <button
            key={idx}
  ...
  for (let i = 0; i < count; i++) {
    ...
    users.push({
      ...
      stories: Array.from({ length: storyCount }, (_, storyIdx) => {
        const type = ['image', 'video', 'text', 'component'][Math.floor(Math.random() * 100) % 4];
        ...
        if (type === 'text') {
          const backgrounds = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8'];
          return {
            id: `story-${i}-${storyIdx}`,
            type: 'text' as const,
